...
let itemDetails = function(req, res){
    ...
    connection.query(sql, [id], (err, results)=> {
        ...
        if(results.length === 0){
            return res.status(400).json("There are no ingredients with that Id.")
        }
...
let createItem = function(req, res){
    console.log('INSIDE /POST /create-ingredient')
    const {recipe_id, steps, description} = req.body
    const sql = `
        INSERT INTO ingredients (recipe_id, steps, description)
        VALUES (?, ?, ?);
    `
